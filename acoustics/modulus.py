...
def get_avg_mod(tofs: pd.DataFrame, cell_props: dict, w: list) -> pd.DataFrame:
    ...
    avg_mod = [cell_props['mass'] / (cell_props['height'] * cell_props['length'])] * len(tofs)
    for i in range(len(tofs)):
        avg_mod[i] *= w[i] / tofs_squared[i]

    return pd.DataFrame(avg_mod)

def linear_cell_thickness(len_ToF: int, w_f: float, w_i: float = 2E-3) -> np.array:
...
def sinusoidal_cell_thickness(w_linear: np.array, tofs: list, n_cycles: int, intracycle_max_thickness: float) -> np.array:
    ...
    return w_linear + (w_linear[-1] - w_linear[0]) * abs(sin_components * intracycle_max_thickness)
...
def get_avg_wavelength(mod: pd.DataFrame, f: float, m: float, l: float, h: float, w: np.array):
...
def normalize_moduli(moduli: list, effective_mod: float) -> list:
    ...
    for modulus in moduli:
        modulus_normalized = modulus.values - modulus.iloc[0].values + effective_mod * 1E9
...
def get_anode_thickness_change_rate(I: float, rho: int, A: float, z: int = 1, M: float = 6.9E-3) -> float:
...
def get_modulus(params):
    ...
    # ToF
    dt, acoustic_amplitudes = backend.get_acoustics(
    ...
    w_linear = linear_cell_thickness(
        w_i=params['w'][0],
        w_f=params['w'][-1],
        len_ToF=len(tofs)
    )
    ...
    sinusoidal_mod = get_avg_mod(
        tofs=tofs,
        cell_props=cell_props,
        w=w_sinusoidal
    )

    figures.plot_thickness_models(
        dt=dt,
