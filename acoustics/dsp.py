...
def absolute_tof(waves: np.array,
                 exp_params: dict,
                 t_sta: float = 0.05) -> list:
    ...
    nlta = exp_params[
        'lta2sta'] * nsta  # Length of long time average window in samples
    ...
    for i, wave in enumerate(waves):
        ...
        stalta = classic_sta_lta(a=wave, nsta=int(nsta), nlta=int(nlta))

        threshold = 0.75 * np.nanmax(
            stalta)  # We lover the threshold a little bit

        # threshold for first break picker
        trigger_index = (stalta > threshold).argmax() if (
            stalta > threshold).any() else -1
        absolute_ToFs[i] = exp_params['t_0'] - exp_params['t_ref'] + (
            trigger_index / no_samples_per_pass) * (exp_params['t_1'] -
                                                    exp_params['t_0'])
...
def tof_shift(waves: np.array,
              delay: float = 26.6,
              duration: float = 6.0,
              points: int = 10000,
              kind='cubic',
              tolerance=3.,
              fill: int = 128) -> np.array:
    ...
    tofs = np.linspace(delay, delay + duration,
                       len(waves[0]))  # Linearly spaced
    t_resampled = np.linspace(delay, delay + duration, int(points))
    wave_reference = scipy.interpolate.interp1d(x=tofs,
                                                y=waves[0],
                                                kind=kind,
                                                bounds_error=False,
                                                fill_value=fill)
    ...
    for i, wave in enumerate(waves):
        wave_being_inspected = scipy.interpolate.interp1d(x=tofs,
                                                          y=wave,
                                                          kind=kind,
                                                          bounds_error=False,
                                                          fill_value=fill)
...
class ToFShift:
    ...
    def get_range(self, cross_t) -> [int, int]:
        ...
        range_low = self.old_index - 0.5 * self.tolerance / self.time_span * len(
            cross_t)
        range_high = self.old_index + 0.5 * self.tolerance / self.time_span * len(
            cross_t)
